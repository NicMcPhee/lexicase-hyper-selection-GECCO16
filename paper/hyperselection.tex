...
\todo[inline]{Tom: The last sentence here seems really obfuscated. I should know, I wrote it. Any ideas about conveying the same info without being confusing?}
...
It is perhaps somewhat surprising that SLT and lexicase selections lead to such similar error diversities, as we might expect the lexicase hyperselection events to cause major drops in population diversity. Looking at the behavior of individual lexicase runs such as those in Figure~\ref{fig:rswnBounceback} we see, at least in these examples, that lexicase is able to increase error diversity quite quickly, including shortly after major drops corresponding to hyperselection events. The consistently high levels of error diversity when using lexicase (e.g., Figures~\ref{fig:dl-div}--\ref{fig:rswn-div}) suggest at the very least that if hyperselection leads to drops in diversity, lexicase selection is then generally able to promptly restore diversity.
...
\begin{figure}[t]
	...
	\caption{\textbf{THIS IS NOT THE ACTUAL GRAPH THAT I'LL INCLUDE; I'LL SWAP THAT ONE IN TONIGHT WHEN I GET HOME! -- Nic.} Error diversity for three lexicase runs of Replace Space With Newline, showing the rapid return of diversity after hyperselection events.}
