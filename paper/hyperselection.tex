...
\todo[inline]{Also, this is where some other selection methods should be briefly mentioned, with citations, so we're not dinged for neglecting related work, but then we say: Because the aim of this paper is only to investigate the relationship between hyperselection and lexicase selection, and not to compare lexicase selection to other selection methods, further discussion of alternate selection methods is outside the scope of this paper.}

%- no verbatim copying (besides dissertation)

Lexicase\footnote{The term ``lexicase'' has been used previously in unrelated work~\cite{Starosta:1986:LPL:991365.991400, starosta1988case}.} selection is a method for selecting individuals to serve as parents in population-based stochastic search algorithms such as GP. As a behavior-based search driver~\cite{Krawiec:2015:GPTP}, it can be used any time that potential parents are assessed with respect to multiple test cases.
Previous work has shown that lexicase selection can effectively increase performance while also increasing behavioral diversity on a variety of GP problems~\cite{Helmuth:2014:ieeeTEC, Helmuth:2014:GECCO, Krawiec:2015:GECCO:smgpWorkshop, Helmuth:2013:GECCOcomp}.

The lexicase selection algorithm is outlined in Figure~\ref{lexicase}. The key idea is that the test cases are randomly shuffled for \emph{each} selection event, and then considered in that order. For each test case, the only individuals that are kept are those whose error is minimal \emph{among those still under consideration}. This filtering is then repeated for each test case until there is only a single individual left or until all the test cases have been considered, in which case an individual is randomly chosen from the remaining pool.
...
Because the ordering of the test cases is randomized for each selection, the priority of test cases is different for each selection. Assuming the population size is considerably larger than the number of test cases, this ensures that each test case is most important (first in the order) for several of the selections, and very important (in the early part of the order) for many more selections. The idea, then, is that this can help maintain population diversity by ensuring that the population does not converge on solutions that focus on a subset of easy test cases while ignoring the harder ones. This has been shown to be true in a number of software synthesis benchmark problems, where lexicase selection substantially outperforms tournament selection and implicit fitness sharing~\cite{Helmuth:2015:GECCO,Helmuth:2015:dissertation} and typically maintains higher levels of diversity~\cite{Helmuth:2015:GPTP}.

%- characteristics (brief)


%- cite previous results
% - performance results


% - diversity results (both lexicase paper and dissertation)
...
\todo[inline]{Tom: Should we add which results are significantly better than tournament? I had those in a different table in my thesis, but it seems like it would be worthwhile to have here.
	...
	Nic: Can you just add an asterisk next to the problem names where the number of successes is statistically significantly better, and then note it in the caption? That would seem to be nice.}
